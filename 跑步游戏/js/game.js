...
person.prototype={
    draw:function(){
        ...
        this.cobj.drawImage(this[this.status][this.state], 0, 0, 400, 400, 0, 0, this.width, this.height);
        this.cobj.restore();
    },
    update:function(){
        if (this.y > this.endy) {
...
function game(canvas, cobj, runImg, jumpImg,hinderImg) {
    /*得分*/
    $("<div class='box1'>得分：0</div>").css({
        width:228,height:63,position:"absolute", right:50,top:80,background:"url(images/startbtn.png)",fontSize:"18px",textAlign:"center",lineHeight:"53px",fontFamily:"微软雅黑",color:"#fff"
    }).appendTo("body");
    /*关卡*/
    $("<div class='guanka'>关卡：1</div>").css({
        width:228,height:63,position:"absolute",right:50,top:140,background:"url(images/startbtn.png)",fontSize:"18px",textAlign:"center",lineHeight:"53px",fontFamily:"微软雅黑",color:"#fff"
    }).appendTo("body");
    $("<div class='sheng'>生命：3</div>").css({
        width:228,height:63,position:"absolute", right:50,top:200,background:"url(images/startbtn.png)",fontSize:"18px",textAlign:"center",lineHeight:"53px",fontFamily:"微软雅黑",color:"#fff"
    }).appendTo("body");
    $("<div class='stop'>暂停</div>").css({
        width:228,height:63,position:"absolute", right:50,top:260,background:"url(images/startbtn.png)",fontSize:"18px",textAlign:"center",lineHeight:"53px",fontFamily:"微软雅黑",color:"#fff"
    }).appendTo("body");
    $(".stop").click(function(){
        alert(1)
    })
    $(".endx_game_go").click(function(){
        $(this).fadeOut(300);
        location.reload();
    })
    $(".endx_game_end").click(function(){
        $(this).fadeOut(300);
            ...
            for(var i=0;i<that.hinderArr.length;i++){
                ...
                if(hitPix(that.canvas,that.cobj,that.person,that.hinderArr[i])){
                    ...
                    if(!that.hinderArr[i].flag&&!that.hinderArr[i].flag1) {
                        ...
                        that.score++;
                        $(".box1").html("得分："+that.score);
                        if(that.score>=that.stage*10){
                            that.stage++;
                            $(".guanka").html("关卡："+that.stage);
                        }
                        if(that.score%3==0){
                            that.speed+=1;
                        }
